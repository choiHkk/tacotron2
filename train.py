...
def prepare_dataloaders(hparams):
    ...
    train_loader = DataLoader(
        trainset, num_workers=1, shuffle=True,
...
def prepare_directories_and_logger(hparams, output_directory, log_directory,
    ...
    logger = Tacotron2Logger(tensorboard_log_dir_name, terminal_log_file_path, hparams)
    return logger, model_state_dir_name


def load_model(hparams, gpu):
    from model import Tacotron2
    model = Tacotron2(hparams).cuda(gpu)
    return model
...
def train(gpu_nums, output_directory, log_directory, checkpoint_path,
    ...
    criterion = Tacotron2Loss().cuda(main_gpu)
...
def synthesizer_train(args):
    ...
    main model training:
        python train.py -g 0 -o data -l kss_v1 -d /home/work/tacotron2/data/dataset/kss
